...
const float ROTATION_ANGLE = M_PI / 18; // 10 degrees in radians
...
void renderTriangle(SDL_Renderer* renderer, const Triangle3D& triangle, const Point3D& cameraPosition) {
    ...
    // Adjust intensity based on depth
    int intensity = static_cast<int>(255 * (1.0f - std::min(depth / 300.0f, 1.0f))); // Scale depth to intensity
            ...
            if (event.type == SDL_QUIT) {
                ...
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_RIGHT:
                        triangle.translate(Point3D(MOVE_STEP, 0, 0));
                        break;
                    case SDLK_r: // Rotate around X-axis
                        triangle.rotate(ROTATION_ANGLE, 'x', centroid);
                        break;
                    case SDLK_t: // Rotate around Y-axis
                        triangle.rotate(ROTATION_ANGLE, 'y', centroid);
                        break;
                    case SDLK_y: // Rotate around Z-axis
